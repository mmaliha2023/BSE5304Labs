...
# gdal_polygonize.py -8 mydemw.tif mydemw_poly_gdal.shp
 ...
 url = "https://websoilsurvey.sc.egov.usda.gov/DSD/Download/AOI/kbp1kbwvqa0qivz2bvzhmw0f/wss_aoi_2023-02-14_12-43-46.zip"
 # this url is soil map downloaded from websoilsurvey for Deadrun watershed at MD
 ...
 mysoil=readOGR("wss_aoi_2023-02-14_12-43-46/spatial/soilmu_a_aoi.shp")    
...
TMWB$AWC = (0.45-0.15)*1000 #Fld Cap = 0.45, Wilt pt = 0.15, z=1000 m
TMWB$dP = TMWB$P - TMWB$ET
 ...
 ################################################################################### 
  ...
  View(TMWB)
  dev.off()
  plot(TMWB$date,TMWB$Qmm,col="red",ylab ="Qmm(mm)",xlab="date",type="l")
  lines(TMWB$date,TMWB$Qpred,col="blue",type="l", 
          xlab = "", ylab = "")
  legend("topright", c("Qmm(mm)", "Qpred(mm)"), col = c("red", "blue"),
           lty = 1:2, cex = 0.8)
 ...
 ggplot() + geom_line(aes(x=TMWB$date,y=TMWB$P),color='red') + 
   geom_line(aes(x=TMWB$date,y=TMWB$ET),color='blue') + 
   geom_line(aes(x=TMWB$date,y=TMWB$AW),color='orange') + 
   geom_line(aes(x=TMWB$date,y=TMWB$Excess),color='green') +
   geom_line(aes(x=TMWB$date,y=TMWB$Qpred),color='pink') + 
   geom_line(aes(x=TMWB$date,y=TMWB$Qmm),color='black') + 
   ylab('P/ET/AW/Excess/Qpred/Qmm in mm')+xlab('Year') +
   scale_color_manual(name ="",breaks = c("TMWB$P", "TMWB$ET", "TMWB$AW", "TMWB$Excess", "TMWB$Qpred", "TMWB$Qmm"),values = c("red", "blue", "orange", "green", "pink", "black"))+
   theme(legend.position = c(0, 1),legend.justification = c(0, 1))+
   ggtitle('Homework1: Dear Run Watershed at Franklintown, MD')
   
 #####################################
   attach(TMWB)# Remember to detach or it gets ugly
   plot(date,Qmm,type = "l",col="black", xlab = "Year", ylab="P/ET/AW/Excess/Qpred/Qmm in mm")
   lines(date,P,type = "l",col="red")
   lines(date,Qmm,type = "l",col="black") # We repeat to have Qmm on top of P
   lines(date,ET,type = "l",col="blue")
   lines(date,Excess,type = "l",col="green")
lines(date,Qpred,type = "l",col="pink")
lines(date,AW,type = "l",col="orange")
title("Homework1: Dear Run Watershed at Franklintown, MD")


   legend("topright", c("P", "Qmm", "ET", "Excess", "Qpred", "AW"), col = c("red", "black", "blue", "green", "pink", "orange"),
          lty = 1:2, cex = 0.8)
   detach(TMWB) # IMPORTANT TO DETACH
