...
#
myflowgage_id="01644372"  #LITTLE SENECA CREEK TRIBUTARY AT BRINK, MD
...
mu2chmax$ksat_mmhr = mu2chmax$ksat_r*3600/1000
...
MUSLE=aggregate(MUSLE_mrg,list(MUSLE_mrg$ID),mean,na.rm=T) # to aggregate based on HRU ID
...
for(i in 1:5){
  ...
  tmpdf$sed=(tmpdf$Qpred*tmpdf$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE$KCPLSCFRG118[i] 
  assign(objname,tmpdf)
}
View(tmpdf)
plot(tmpdf$date, tmpdf$sed)

#Instead of TIClass, the loop can be done through HRUID, making the model spatially distributed
...
#################################### P Loss due to baseflow for the whole basin #################
...
#find DP for each TI class Then do the following to get the P loss
...
DPLT$LT
plot(DPLT$date,DPLT$LT, type="l",ylim=c(0,0.25), main = "", xlab="Date (Year)", ylab="Watershed P export (kg/day)",
     col.axis="blue") 
title(main = "Time series of Watershed P export", 
      cex.main = 2,   font.main= 1, col.main= "black",sub = "original",
...
reclass_m <- matrix(reclass_df,
                    ncol = 3,
                    byrow = TRUE)
dplt_classified <- reclassify(TI,
                             reclass_m)
dplt_classified[dplt_classified == 0] <- NA
plot(dplt_classified,
     col = c("red", "blue", "green"))
title (main="TIC raster map for mean annual surface P export")
mtext("P Loss * 10 ^-2 kg/day", side=4, line=0)
