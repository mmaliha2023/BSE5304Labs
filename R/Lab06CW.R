...
# What is going to change from use case to use case 
 LabNo="/Lab06"
 myflowgage_id="01589330"  # Old Friendly Gage
#
...
#            CN,  Ia, Slp, Asp, AWC,Depth,ResCoef
 lower <- c(35,0.01, 0.0, 0.0, 0.2,  500,   0.1)
...
######################################################################################
 ...
 # setup and train the neural network
  net <- neuralnet(Qm3ps ~ Qm3ps_lag+Precip+jdate+Jday+TMX+TMN,
                    data = train,
                    hidden = c(6, 2), # can adjust here
                    threshold = 0.0082, # if you network does not run, 
                    #                         try turning this up just a little
                    stepmax = 1e+03, # can adjust if you don't converge
                    rep = 1,
                    #startweights = NULL,
